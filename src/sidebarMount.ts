...
function hubLog(...args: unknown[]): void {
...
function ensureContainer(): HTMLElement {
    ...
    const matchedSelector = SIDEBAR_SELECTORS.find(sel => document.querySelector<HTMLElement>(sel));
    const sidebar = matchedSelector ? document.querySelector<HTMLElement>(matchedSelector) : null;
    const parent = sidebar?.parentElement;
    if (parent) {
        hubLog('mount: sidebar found via', matchedSelector);
        parent.insertBefore(div, sidebar);
        return div;
    }

    hubLog('mount: sidebar not found, using fixed fallback');
    // フォールバック: 画面右下に固定表示
